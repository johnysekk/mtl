// /api/pis-return.js — the redirect_url the student lands on AFTER approving in their bank.
// This is the SYNCHRONOUS confirmation (the webhook is the reliable async one). We GET the payment
// status and, if final-success, mark the booking paid (idempotent with the webhook), then bounce
// the student back into the app.
...
export default async function handler(req, res){
  try{
    // Enable appends payment_id + state (our bookingId) to the redirect query string.
    ...
    // not final yet -> the webhook will confirm; send the student back with a "pending" hint

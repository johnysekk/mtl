...
async function pisSideEffects(rec, tbl){
  ...
  try{ if(_event){ let target=(_evP==='coach')?_evC:null; if(!target && _evG){ const g=await sb.from('gyms').select('owner_id').eq('id',_evG).maybeSingle(); target=g.data&&g.data.owner_id; } if(target) await sb.from('notifications').insert({ user_id:target, type:'booking', read:false, message:'\ud83c\udf9f\ufe0f Nov\u00fd prodej vstupenky (p\u0159evodem): '+(rec.buyer_name||'Z\u00e1kazn\u00edk'), data:JSON.stringify({ kind:'pis_payment_in', event_id:rec.event_id, student:(rec.buyer_name||''), amt:(rec.amount!=null?String(rec.amount):''), sym:(rec.currency||'CZK') }) }); }
      ...
      // neříká jen "Student".
      let _who1 = rec.student_name || '';
      if(!_who1 && rec.student_id){ try{ const _sp = await sb.from('profiles').select('name').eq('id', rec.student_id).maybeSingle(); _who1 = (_sp.data && _sp.data.name) || ''; }catch(e){} }
      await sb.from('notifications').insert({ user_id:rec.coach_id, type:'booking', read:false, message:'\ud83d\udcb3 Nov\u00e1 1:1 platba (p\u0159evodem)'+((rec.amount!=null)?(' \u00b7 '+rec.amount+' '+(rec.currency||'CZK')):''), data:JSON.stringify({ kind:'pis_payment_in', coach_id:rec.coach_id, booking_id:rec.id, student:_who1, amt:(rec.amount!=null?String(rec.amount):''), sym:(rec.currency||'CZK'), date:(rec.training_date||null), time:(rec.training_time||null) }) }); }
...
export default async function handler(req, res){
  ...
  try{
    ...
    if(!paymentId && bookingId){
      ...
      // 1:1 lekce. Sloupec pis_payment_id na 'bookings' dřív nebyl, takže se sem PIS platba
      // nedala dohledat a spadla na ruční potvrzení koučem -- přestože ji banka potvrdila.
      if(!r0){ const b0=await sb.from('bookings').select('pis_payment_id').eq('id',bookingId).maybeSingle(); if(b0.data) r0=b0.data; }

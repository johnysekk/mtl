  ...
  try{
    ...
    if(paymentId){
      ...
      if(r.ok && PAID_STATUSES.has(String(status))){
        ...
        let rec=(await sb.from('gym_bookings').select('id,status,student_id,gym_id,class_name,class_date,class_time,amount,coach_id,acq_source,student_name').eq('pis_payment_id',paymentId).maybeSingle()).data;

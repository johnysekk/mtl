...
// the gym owner's rate so a gym cannot tamper it, then writes the row with the
// service-role key. This lets RLS lock the transactions table to server-only inserts.
//
// Body: { token, gym_id, member_id?, coach_id?, gross_amount(minor), currency?,
//         type('drop_in'|'membership'|'custom'), payment_method('cash'|'qr'),
//         cash_payer_name?, acq_source? }
// Auth: token must be the gym OWNER's Supabase access token.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { token, gym_id, member_id, coach_id, gross_amount, currency, type, payment_method, cash_payer_name, acq_source } = b;

    if (!token || !gym_id || !type || !payment_method) return res.status(400).json({ error: 'missing fields' });
    if (!['cash', 'qr'].includes(payment_method)) return res.status(400).json({ error: 'bad method' });
    if (!['drop_in', 'membership', 'custom'].includes(type)) return res.status(400).json({ error: 'bad type' });
    ...
    // gym + authorize (owner records cash)
    ...
    const row = {
      gym_id, member_id: member_id || null, coach_id: coach_id || null, paid_to: 'gym',
    ...
    return res.status(200).json({ ok: true, mtl_fee, id: (ins && ins[0] && ins[0].id) || null });
  } catch (e) {

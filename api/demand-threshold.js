...
export default async function handler(req, res) {
  try {
    ...
    const rows = await sbGet(`demand_signals?select=user_id,city,country,disciplines,lat,lng,source,opens&source=neq.resolved&last_seen_at=gte.${fresh}&lat=gte.${lat - dLat}&lat=lte.${lat + dLat}&lng=gte.${lng - dLng}&lng=lte.${lng + dLng}&limit=8000`);

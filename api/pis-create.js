...
export default async function handler(req, res) {
  ...
  try {
    const {
      ...
      aspspCountry = 'CZ',
      psuType = 'personal',
      kind                  // 'memb'->gym_memberships, 'coach1'->bookings, 'event'->event_tickets, 'cohort'->cohort_members, 'merch'->merch_orders, else gym_bookings
    } = req.body || {};
    ...
    try {
      const _pisTbl = (kind === 'memb') ? 'gym_memberships' : (kind === 'coach1') ? 'bookings' : (kind === 'event') ? 'event_tickets' : (kind === 'cohort') ? 'cohort_members' : (kind === 'merch') ? 'merch_orders' : 'gym_bookings';
